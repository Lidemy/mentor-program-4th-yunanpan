...
function getGames(fn) {
  ...
  }).then(response => response.json()).then(json => fn(json)).catch(err => console.log('error', err));
}

// 2. 指定遊戲前 20 名的直播
function getStreams(gameName, fn) {
  fetch(`${streamURL}?game=${encodeURIComponent(gameName)}`, {
    headers: {
      Accept: 'application/vnd.twitchtv.v5+json',
      'Client-ID': 'yoshvqznzi8j2ild4h8k8il4iultit',
    },
  }).then(response => response.json()).then(json => fn(gameName, json)).catch(err => console.log('error', err));
}
